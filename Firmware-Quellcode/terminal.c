...
static uint8_t enternumber(void) {
    ...
    for (uint8_t i = 0; i < 3; i++) {
        ...
        if((num >= '0' && num <= '9') || (num == 10) || (num == 13)) entry[i] = num > 13 ? num : 0;

        if(!entry[i]) break;
        else uart_putc(entry[i]);
    }
    sscanf(entry, "%u", &number);

    if(!number || (number > 15)) return 0;
    ...
    if(swpos != 's') {
        ...
        switch (valid | 0x20) {
            ...
            case 'm': {
                ...
                if((stmode | 0x20) == 'f') use_variable_intervalls[swpos] = 0;

                if((stmode | 0x20) == 'v') use_variable_intervalls[swpos] = 1;

                if(ivalnr != use_variable_intervalls[swpos]) eewrite(use_variable_intervalls[swpos], swpos + 2);
...
void list_complete(uint16_t *variable_intervalls, uint16_t *fixed_intervalls, uint8_t *use_variable_intervalls) {
    ...
    if(!swpos) uart_puts(TERM_COL_YELLOW);
    else uart_puts(TERM_COL_WHITE);

    uart_puts_P(PSTR(" S           Fixed: Trigger\r\n\n"));

    for(uint8_t i = 0; i < 11; i++) {
        if(i == swpos - 1) uart_puts(TERM_COL_YELLOW);
        else uart_puts(TERM_COL_WHITE);
